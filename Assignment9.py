...
storyConsecutiveDictionary = {}
i = 0
while(i < (len(storyArray)-1)):
    if storyArray[i]+" "+storyArray[i+1] in storyConsecutiveDictionary:
        storyConsecutiveDictionary[storyArray[i]+" "+storyArray[i+1]] += 1
    else:
        storyConsecutiveDictionary[storyArray[i]+" "+storyArray[i+1]] = 1
    i += 1
...
# grab the last 5 elements in the dictionary
storySortedCount = sorted(
    storyConsecutiveDictionary.items(), key=operator.itemgetter(1))
